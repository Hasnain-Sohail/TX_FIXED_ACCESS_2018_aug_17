...
public class Problem_ZTE_TXM_Access_Fiber_Cut extends PTCL_Base_Problem {

	private static final Logger LOG = LoggerFactory.getLogger(Problem_ZTE_TXM_Access_Fiber_Cut.class);
	...
	public List<String> computeProblemEntity(Alarm a) throws Exception {
		...
		String AEnd = MapperUtils.doMapping(a, "retrieveZTEAEndFrom");
		String ZEnd = MapperUtils.doMapping(a, "retrieveZTEZEndTo");
		
		if(!StringUtils.isEmpty(AEnd) 
				&& !StringUtils.isEmpty(ZEnd)) {
			
			String pe = "";
			List<String> sortedList = CalculateUtil.getSortedStringList(AEnd.toLowerCase(), ZEnd.toLowerCase());
			...
			if(!StringUtils.isEmpty(pe)) {
				...
				LOG.debug(String
						.format("computeProblemEntity pe is [%s] for Problem Problem_ZTE_TXM_Access_Fiber_Cut alarm is [%s]",
								pe,
								a.getIdentifier()
								));
